...
class TestDriver(base.BaseTestCase):
    ...
    @mock.patch("valence.provision.ironic.utils.create_ironicclient")
    def test_node_register_redfish_failure(self, mock_client, mock_redfish,
                                           mock_db):
        mock_redfish.side_effect = Exception()
        self.assertRaises(exception.ValenceException,
                          self.ironic.node_register,
                          'fake-uuid', {})
    ...
    @mock.patch("valence.redfish.redfish.systems_list")
    @mock.patch("valence.provision.ironic.utils.create_ironicclient")
    def test_node_register(self, mock_client, mock_redfish,
                           mock_node_get, mock_node_update):
        ironic = mock.MagicMock()
        mock_client.return_value = ironic
        mock_redfish.return_value = [{'id': 1}]
        mock_node_get.return_value = {'name': 'test', 'metadata':
        ...
        self.assertEqual({
            ...
            'detail': 'The composed node fake-uuid has been '
                      'registered to Ironic successfully.',
