...
class Connection(object):
    ...
    def __init__(self, podm_url, podm_auth):
        ...
        self.redfish = self.init_refsih('podm_uuid')

    def init_refsih(self, podm_uuid):
        refish = ''  # TODO init redfish class instance by podm
        return refish
    ...
    def list_racks(self, show_detail=True):
        chassis_collection_url = self.podm_url + '/Chassis'
        chassis_collection = http.get_http_request(url=chassis_collection_url,
                                                   http_auth=self.podm_auth)
        raw_racks = self.__filter_chassis(chassis_collection.json(), "Rack")
        racks = []
        filter_passed = True
        ...
        for rack in raw_racks:
            ...
            rack_id = rack["Id"]
            rack_name = rack["Name"]
            rack_systems = self.__get_systems_by_chassis(rack)
            rack_info.update({"id": rack_id,
                              "name": rack_name,
    ...
    def __filter_chassis(self, jsonContent, filterCondition):
        return_members = []
        members = jsonContent['Members']
        for member in members:
            resource = member['@odata.id']
            resp = http.get_http_request(url=resource,
                                         http_auth=self.podm_auth)
            member_detail = resp.json()
            chassis_type = member_detail['ChassisType']
            if chassis_type == filterCondition:
                return_members.append(member_detail)
        return return_members

    def __get_systems_by_chassis(self, chassis):
