...
class ExpEtherManager(object):
    ...
    def create_compose_request(self, name, description, requirements):
        """Create request for compose_node

        :param name: name of node
        :param description: description if any
        :param requirements: additional requirements if any like pci devices
        :return: request in format {'Name': 'name',
    ...
    def node_action(self, node_index, request):
        ...
        LOG.debug("%s device %s to node %s", action, device_db['uuid'],
                  node_index)
    ...
    def detach(self, device_db, node_index=None):
        ...
        if ((device_db['pooled_group_id'] == EEIO_DEFAULT_GID) and
                (device_db['node_id'] is None)):
