...
class ValenceException(ValenceError):
    def __init__(self, detail, status,
        ...
        self.status = status or http_client.SERVER_ERROR
        self.code = "ValenceError"
        self.title = "Valence Server Error"
        self.detail = detail
...
def wrap_keystone_exception(func):
    ...
    def wrapped(*args, **kw):
        try:
            ...
            message = ("%s connection failed. Reason: %s" %
                (func.__name__, sys.exc_info()[1]))
            raise AuthorizationFailure(detail=message)
        except keystone_exceptions.ClientException:
            message = ("%s connection failed. Unexpected keystone client "
                       "error occurred: %s" % (func.__name__, sys.exc_info()[1]))
            raise AuthorizationFailure(detail=message)
    return wrapped
