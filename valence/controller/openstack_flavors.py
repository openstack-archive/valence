        ...
        if isinstance(value, list):
            ...
            if key in extra_spec_value.keys():
                ...
                if (isinstance(value, str) and
                   (value == convert_data_type((extra_spec_value.get(key)),
                                               type(value)))):
                    if noparams == 0:
                        satisfies_spec = True
    ...
    for key, value in node_spec.items():
        if isinstance(value, list):
            ...
            if key == extra_spec_key:
                found_key = True
                if (isinstance(value, str) and
                    (value == convert_data_type(extra_spec_value,
                                                type(value)))):
                    satisfies_spec = True
                    return satisfies_spec, found_key
                elif ((not isinstance(value, str))
                      and (value >= convert_data_type(extra_spec_value,
                                                      type(value)))):
                    satisfies_spec = True
                    return satisfies_spec, found_key
                else:
