...
class RSAPodmVendorPassthru(base.VendorInterface):
    ...
    def get_resource_usage_summary(self, context, **kwargs):
        ...
        for model in ['Disk', 'CPU', 'Memory']:
            for flag in ['total', 'used']:
                string = 'objects.%s.get_%s_sum_by_systems(context, %s_system_id_list)' % (
                model, model.lower(), flag)
    ...
    def inventory_pod_manager_resources(self, context, **kwargs):
        ...
        try:
            ...
            LOG.error(
                "refresh inventory for pod manger: %s failed" % pod_manager.name)
            LOG.error(ex)
    ...
    def get_rack_list(self, context, **kwargs):
        pod_id = kwargs['pod_id']
        chassis_list = objects.RSAChassis.list_by_pod_and_type(context, pod_id,
                                                               constants.CHASSIS_TYPE_RACK)
    ...
    def get_drawer_by_chassis_id(self, context, **kwargs):
        ...
        chassis_info[
            'computer_systems'] = objects.RSAChassis.get_chassis_computer_systems(
            pod_id, chassis, chassis_id,
            context)
    ...
    def get_pcie_switch_info(self, context, **kwargs):
        pod_id = kwargs['pod_id']
        pcie = \
        objects.RSAChassis.list_by_pod_and_type(context, pod_id, 'RackMount')[
            0]
    ...
    def get_computer_system_details_info_by_id(self, context, **kwargs):
        ...
        if pod_type == "INTEL-COMMON":
            ...
            drawer_id = \
            eval(objects.ComputerSystem.get_by_id(context, node_id).location)[
                "Chassis"]
            rack_id = \
            eval(objects.ComputerSystem.get_by_id(context, node_id).location)[
                "Rack"]
    ...
    def get_composed_node_details_info_by_id(self, context, **kwargs):
        ...
        # get system info
        system = self.get_computer_system_details_info_by_id(context,
                                                             pod_id=pod_id,
                                                             system_id=node.computer_system_id)
        system['data']['computer_system_name'] = system['data']['name']
        # get volume info
        volume_list = map(lambda volume_id: objects.Volume.get_by_id(context,
                                                                     volume_id).as_dict(),
                          eval(node.volume_id))
    ...
    def __synchronize_computer_system(self, context, pod_id, system_info):
        ...
        if pod_type == "INTEL-COMMON":
            ...
            rack = \
            eval(objects.RSAChassis.get_by_id(context, chassis.id).location)[
                "Rack"]
    ...
    def __synchronize_composed_node(self, context, pod_id, node_info):
        ...
        node_db.computer_system_id = objects.ComputerSystem.get_by_url(context,
                                                                       computer_system_url).id
    ...
    def __synchronize_volume(self, context, pod_id, volume_info):
        ...
        volume_db.size_GB = volume_info[
            "CapacityGiB"] if "CapacityGiB" in volume_info else volume_info[
                                                                    "CapacityBytes"] / 10 ** 9
        ...
        if pod_type == "INTEL-COMMON":
            storage_url = pod_manager_url + volume_info['Links']['Targets'][0][
                '@odata.id']
            volume_db.controller_id = objects.RemoteTarget.get_by_url(context,
                                                                      storage_url).id
        else:
