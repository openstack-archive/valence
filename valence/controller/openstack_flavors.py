...
def check_spec(node_spec, extra_spec_key, extra_spec_value):
     ...
     for key, value in node_spec.items():
               if isinstance(value,list):
                  for k in value:
                     satisfies_spec, found_key = check_spec(k, extra_spec_key, extra_spec_value)
               elif isinstance(value,dict):
                  satisfies_spec, found_key = check_spec(value, extra_spec_key, extra_spec_value)
               else:
                  if key == extra_spec_key:
                      found_key = True
                      if value == extra_spec_value:
                         print value, extra_spec_value
                         satisfies_spec = True
...
def create_flavor(values):
    ...
    if not flavor_created:
         print "failed to create flavor as no node satisfies extra_specs" 
...
def register(flavoruuid):
    flavor =  db_api.Connection.get_openstack_flavor_by_uuid(flavoruuid).as_dict()
    ...
    resp = cflavor.set_keys(extra_specs)
