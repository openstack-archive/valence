...
class EtcdDriver(object):
    ...
    def list_openstack_flavors(self):
        try:
            ...
            LOG.error("Path '/openstack_flavors' does not exist, the etcd \
                      server may not have been initialized appropriately.")
            raise
    ...
    def get_openstack_flavor_by_uuid(self, flavor_uuid):
        try:
            resp = self.client.read(models.OpenstackFlavor.etcd_path(
                                    flavor_uuid))
        except etcd.EtcdKeyNotFound:
            raise Exception('Flavor {0} not found.'.format(flavor_uuid))
