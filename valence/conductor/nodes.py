...
from valence.conductor.tasks import Task as tasks
...
class Node(object):
    ...
    @classmethod
    def compose_node(cls, request_body, task_uuid):
        """Compose new node
        ...
        logging.notice("\nconductor: Task received"
                       + " with task_uuid %s \n\n" % (task_uuid))
        task = tasks.update_task(task_uuid)
        logging.notice("\nconductor: Task state updated \n%s \n\n"
                       % (task))
        try:
            ...
            # Only store the minimum set of composed node info
            # into backend db,
            # since other fields like power status may be changed
            # and valence is
            # not aware.
            ...
            errlocation = os.path.abspath(frameinfo.filename) + ":"\
                + str(frameinfo.lineno)
            errormessage = ex.message + " " + errlocation
            tasks.update_task(task_uuid, errormessage)
            logging.error("\nconductor: Task failed \n%s\n\n" % (task))
            raise(ex)
        task = tasks.update_task(task_uuid)
        logging.notice("\nconductor: Task processed \n%s\n\n" % (task))
    ...
    def list_composed_nodes(cls):
        ...
        res = [cls._show_node_brief_info(node_info.as_dict())
