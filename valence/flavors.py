...
def list_flavors():
    ...
    flavors = []
    for flavor in flavor_models:
        flavors.append(flavor.as_dict())
    return utils.make_response(http_client.OK, flavors)


def create_flavor(values):
    flavor = db_api.Connection.create_flavor(values)
    return utils.make_response(http_client.OK, flavor.as_dict())
