...
def reset_node(nodeid, action):
    ...
    if node_state not in ["PoweredOn", "PoweredOff"]:
        message = "node %s cannot be reseted as it is in %s state" \
                  % (nodeid, node_state)
        reset_allowed = False
        status_code = 400
    ...
    if action not in reset_allowed_values:
        message = ("action %s cannot be performed to the node %s in %s state"
                   "valid actions are in :%s") % (action, nodeid, node_state,
                                                  reset_allowed_values)
        reset_allowed = False
        status_code = 400

    action_state_conflict_map = {
        "PoweredOn": ["On", "ForceOn"],
        "PoweredOff": ["ForceOff", "GracefulRestart", "ForceRestart", "NMI",
                       "GracefulShutdown"]
    }
    if action in action_state_conflict_map.get(node_state, []):
        message = "ConflictingRequest, cannot '%s' node %s while it is in" \
                  " state %s" % (action, nodeid, node_state)
        status_code = 409
        reset_allowed = False
    ...
    if reset_allowed:
        ...
        resp = send_request(reset_url, method="POST", data=data,
                            headers=headers)
        if resp is None:
            message = ("cannot communicate with pod manager, "
            ...
            resp_dict = exception.confirmation(confirm_detail=resp.content)
            return resp_dict, resp.status_code

    resp_json = {'error': message}
    raise exception.RedfishException(resp_json, status_code)
