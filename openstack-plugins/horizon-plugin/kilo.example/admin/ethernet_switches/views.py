...
from openstack_dashboard.dashboards.admin.ethernet_switches import tabs as admin_tabs
from openstack_dashboard.dashboards.admin.ethernet_switches \
...
class IndexView(tables.DataTableView):
    ...
    table_class = admin_tables.AdminEthernetSwitchTable
    #tab_group_class = admin_tabs.PCIeSwitchTabs
    template_name = 'admin/ethernet_switches/index.html'
    page_title = _("Ethernet Switches")
    ...
    def get_data(self):
        ...
        try:
            ...
            switches_list = api.rsa.ethernet_switches_list(request, pod_id)
        except:
...
class DetailView(tabs.TabView):
    tab_group_class = admin_tabs.SwitchDetailTabs
    template_name = 'admin/ethernet_switches/_detail.html'
    #redirect_url = 'horizon:project:instances:index'
    page_title = _("Switch Details: {{ switch.name }}")
        ...
        try:
            try:
                if 'SELECTED_POD_MANAGER_ID' not in self.request.session:
                    ...
                    pod_id = self.request.session['SELECTED_POD_MANAGER_ID']
                result = api.rsa.get_ethernet_switch_by_id(self.request, switch_id)
            except:
                exceptions.handle(self.request,
                                _('Unable to retrieve switch info.'))
            switch = result['data']
            switch['status_2'] = eval(switch['status'])['Health']
        except:
            import traceback
            traceback.print_exc()
        return switch
    ...
    def get_tabs(self, request, *args, **kwargs):
        switch = self.get_data()
        return self.tab_group_class(request, switch=switch, **kwargs)
