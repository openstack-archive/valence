...
class PooledDevices(object):
    ...
    def update_device_info(cls, podm_id):
        ...
        try:
            ...
            podm_devices = connection.get_all_devices()
            mod_podm_devices = {}
            for device in podm_devices:
                mod_podm_devices[device['resource_uri']] = device
            for db_dev in db_devices:
                podm_dev = mod_podm_devices.get(db_dev['resource_uri'], None)
                if not podm_dev:
                ...
                if db_dev['pooled_group_id'] != podm_dev['pooled_group_id']:
                    ...
                    db_api.Connection.update_device(db_dev["uuid"],
                                                    values)
                    del mod_podm_devices[db_dev['resource_uri']]
                    continue
                del mod_podm_devices[db_dev['resource_uri']]
            # Add remaining devices available in podm_devices
            for dev in mod_podm_devices.values():
                dev['podm_id'] = podm_id
                db_api.Connection.add_device(dev)
            response['status'] = 'SUCCESS'

        except exception.ValenceException as e:
            LOG.debug("Update devices failed with exception %s", str(e))
