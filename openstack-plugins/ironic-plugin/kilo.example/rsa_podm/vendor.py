...
class RSAPodmVendorPassthru(base.VendorInterface):
    ...
    def driver_vendor_passthru(self, context, method, **kwargs):
        """driver vendor passthru all subscribe actions are driver
        vendor passthru all pod manager management actions are driver
        vendor passthru
    ...
    def get_resource_usage_summary(self, context, **kwargs):
        ...
        usage['total_disk'] = objects \
            .Disk.get_disk_sum_by_systems(context, total_system_id_list)
        usage['used_disk'] = objects \
            .Disk.get_disk_sum_by_systems(context, used_system_id_list)
        usage['total_cpu'] = objects \
            .CPU.get_disk_sum_by_systems(context, total_system_id_list)
        usage['used_cpu'] = objects \
            .CPU.get_disk_sum_by_systems(context, used_system_id_list)
        usage['total_memory'] = objects \
            .Memory.get_disk_sum_by_systems(context, total_system_id_list)
        usage['used_memory'] = objects \
            .Memory.get_disk_sum_by_systems(context, used_system_id_list)
    ...
    def get_pod_manager_hardware_summary(self, context, **kwargs):
        ...
        return {
            'data': {
                'Racks': len(
                    objects.RSAChassis.list_by_pod_and_type(context,
                                                            pod_id,
                                                            'Rack')),
                'Drawer': len(
                    objects.RSAChassis.list_by_pod_and_type(context,
    ...
    def get_drawer_by_chassis_id(self, context, **kwargs):
        ...
        rack_url = pod_manager_url + '/redfish/v1/Chassis/' + eval(
            chassis_info['location'])['Rack']
    ...
    def get_computer_system_details_info_by_id(self, context, **kwargs):
        ...
        if pod_type == "INTEL-COMMON":
            drawer_url = pod_manager_url + '/redfish/v1/Chassis/' + eval(
                system.location)['Chassis']
            rack_url = pod_manager_url + '/redfish/v1/Chassis/' + eval(
            ...
            drawer_id = eval(
                objects.ComputerSystem.get_by_id(context,
                                                 node_id).location)["Chassis"]
            rack_id = eval(
                objects.ComputerSystem.get_by_id(context,
                                                 node_id).location)["Rack"]
    ...
    def __synchronize_computer_system(self, context, pod_id, system_info):
        ...
        pod_manager_url = self.__get_pod_manager(context, pod_id).url
        try:
            manager_url = pod_manager_url + system_info["Links"][
                "ManagedBy"][0]["@odata.id"]
            chassis_url = pod_manager_url + system_info["Links"][
                "Chassis"][0]["@odata.id"]
        except Exception:
            manager_url = ''
            chassis_url = ''
    ...
    def __synchronize_composed_node(self, context, pod_id, node_info):
        pod_type = self.__get_pod_manager(context, pod_id).type
        pod_manager_url = self.__get_pod_manager(context, pod_id).url
        computer_system_url = pod_manager_url + node_info[
            'Links']['ComputerSystem']['@odata.id']

        if pod_type == "INTEL-COMMON":
            volume_list = node_info["Links"]["LocalDrives"]
        else:
            volume_list = node_info['Oem']['Lenovo:RackScale'][
        ...
        node_db.computer_system_id = objects.ComputerSystem.get_by_url(
            context, computer_system_url).id
        node_db.volume_id = self.__get_volume_ids(
            context, pod_manager_url, volume_list)
    ...
    def __synchronize_rsa_chassis(self, context, pod_id, chassis_info):
        ...
        pod_manager_url = self.__get_pod_manager(context, pod_id).url
        try:
            manager_url = pod_manager_url + chassis_info[
                "Links"]["ManagedBy"][0]["@odata.id"]
        ...
        if pod_type == "INTEL-COMMON":
            ...
            Rack = chassis_info['Oem']['Intel:RackScale']['Location'][
                "ParentId"]
            try:
                ULocation = chassis_info['Oem'][
                                'Lenovo:RackScale']['Location'][
                                "LowerUnit"] + ' U'
                UHeight = chassis_info['Oem'][
                              'Lenovo:RackScale']['Location']["Height"] + ' U'
            except Exception:
                ULocation = ""
                UHeight = ""
            chassis_db.location = {"ULocation": ULocation,
                                   "UWidth": "",
                                   "Rack": Rack,
                                   "UHeight": UHeight}
    ...
    def __synchronize_volume(self, context, pod_id, volume_info):
        ...
        if pod_type == "INTEL-COMMON":
            storage_url = pod_manager_url + volume_info['Links']['Targets'][0][
                '@odata.id']
            volume_db.controller_id = objects.RemoteTarget.get_by_url(
                context, storage_url).id
        else:
