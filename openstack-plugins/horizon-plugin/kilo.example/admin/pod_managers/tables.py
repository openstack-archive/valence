...
class AdminPodManagerFilterAction(tables.FilterAction):
...
class LenovoPhysicalServersCell(Cell):
...
class AdminRefreshPhysicalServer(tables.LinkAction):
    ...
    url = "horizon:admin:pod_managers:refresh"
    classes = ("ajax-modal",)
    icon = "refresh"
    ajax = True
...
class AdminSelectPodManager_old(tables.LinkAction):

    name = "selectpodmanager"
    verbose_name = _("Select this Pod Manager")
    url = "horizon:admin:physical_servers:hypervizor"
    icon="pencil"
    classes = ('btn-danger',)

    def allowed(self, request, pod_manager):
        return True

class AdminRemovePodManager(tables.BatchAction):
    name = "remove_podmanager"
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Remove Pod Manager complete",
            u"Remove Pod Managers complete",
            count
        )
    ...
    def action(self, request, obj_id):
        return_value = api.rsa.unmanage_pod_manager(request, obj_id)

class AdminSelectPodManager(tables.BatchAction):

    name = "batch_select_podmanager"
    verbose_name = _("Select")
    action_present = _("Select")
    action_past = _("Pod Manager selected !")
    #icon="pencil"
    #data_type_singular = _("Pod Manager")
    #data_type_plural = _("Pod Managers")
    classes = ('btn-reboot',)
    #help_text = _("This pod manager will be selected")
    ...
    def action_past(count):
        return ungettext_lazy(
            u"Selected this Pod Manager",
            u"Selected this Pod Managers",
            count
        )

    def allowed(self, request, podmanager=None):
        #if str(pod_managers.SELECTED_POD_MANAGER_ID) == str(podmanager.id):
        #    return False
        if 'SELECTED_POD_MANAGER_ID' in request.session and str(request.session['SELECTED_POD_MANAGER_ID']) == str(podmanager.id):
            return False
        else:
            return True

    def action(self, request, obj_id):
        pod_managers_list = api.rsa.pod_managers_list(request)
        for pm in pod_managers_list:
            if str(pm.id) == str(obj_id):
                request.session['SELECTED_POD_MANAGER_NAME'] = pm.name
                #pod_managers.SELECTED_POD_MANAGER_NAME = pm.name
        #pod_managers.SELECTED_POD_MANAGER_ID = obj_id
        request.session['SELECTED_POD_MANAGER_ID'] = obj_id


class NewPodManagerLink(tables.LinkAction):
    name = "addnew"
    verbose_name = _("Add New POD Manager")
    ...
    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(NewPodManagerLink, self).__init__(attrs, **kwargs)
...
class EditPodManagerLink(tables.LinkAction):
    name = "editpodmanager"
    verbose_name = _("Edit")
    url = "horizon:admin:pod_managers:edit_pod_manager"
    classes = ("ajax-modal", "btn-launch", "icon-pencil-old")
    icon = "pencil"

    def __init__(self, attrs=None, **kwargs):
        kwargs['preempt'] = True
        super(EditPodManagerLink, self).__init__(attrs, **kwargs)
...
class AdminPodManagersTable(tables.DataTable):
    TASK_STATUS_CHOICES = (
        (None, True),
        ("none", True)
    )
    ...
    '''
    ifselected = tables.Column("ifselected",
                         verbose_name=_("If selected"))
    '''
    '''
    ...
    class Meta(object):
        name = "pod_managers"
        verbose_name = _("Pod_managers")
        table_actions = (AdminPodManagerFilterAction,
                         NewPodManagerLink,
                         AdminRefreshPhysicalServer)
        row_actions = (AdminSelectPodManager, EditPodManagerLink,  AdminRemovePodManager, )
        multi_select = False
        '''
