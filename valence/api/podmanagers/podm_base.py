...
class PodManagerBase(object):
    ...
    def __init__(self, username, password, podm_url):
        ...
        self.redfish = sushy_instance.RedfishInstance(username=username,
                                                      password=password,
                                                      base_url=podm_url)
    ...
    def get_system_collection(self):
        ...
        return self.redfish.get_resources_by_url(system_collection_url)

    def get_system_info(self, system_id):
        system_url = self.podm_url + '/Systems/' + system_id
        return self.redfish.get_resources_by_url(system_url)

    def get_node_collection(self):
        node_collection_url = self.podm_url + '/Nodes'
        return self.redfish.get_resources_by_url(node_collection_url)

    def get_node_info(self, node_id):
        node_url = self.podm_url + '/Nodes/' + node_id
        return self.redfish.get_resources_by_url(node_url)

    def compose_new_node(self, podm_compose_request_body):
        compose_url = self.podm_url + '/Nodes/Actions/Allocate'
        # TODO(hubian): maybe the two request body has some difference
        redfish_compose_node_request_body = podm_compose_request_body
