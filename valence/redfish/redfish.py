...
def get_nodebyid(node_index, show_detail=True):
...
def compose_node(request_body):
    ...
    nodes_resp = send_request(nodes_url, 'GET')
    ...
    resp = send_request(allocate_url, 'POST',
    ...
    node_url = resp.headers['Location'].lstrip(cfg.podm_url)
    ...
    node_resp = send_request(node_url, "GET")
    ...
    return get_nodebyid(node_index, show_detail=False)
...
def list_nodes():
    ...
    for url in node_url_list:
        ...
        nodes.append(get_nodebyid(node_index, show_detail=False))
