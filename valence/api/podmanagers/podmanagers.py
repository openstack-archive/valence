...
def get_podm_list():
    return map(lambda x: x.as_dict(), api.Connection.list_podmanager())


def get_podm_by_uuid(uuid):
    return api.Connection.get_podmanager_by_uuid(uuid).as_dict()


def create_podm(values):
    values = _check_creation(values)
    return api.Connection.create_podmanager(values).as_dict()


def update_podm(uuid, values):
    values = _check_updation(values)
    return api.Connection.update_podmanager(uuid, values).as_dict()


def delete_podm_by_uuid(uuid):
    # TODO(hubian) this need to break the links between podm and its Nodes
    return api.Connection.delete_podmanager(uuid)
