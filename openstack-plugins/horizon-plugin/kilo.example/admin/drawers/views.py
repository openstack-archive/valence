...
from openstack_dashboard.dashboards.admin.drawers import tabs as admin_tabs
from openstack_dashboard.dashboards.admin.drawers \
...
class IndexView(tables.DataTableView):
    ...
    table_class = admin_tables.AdminDrawersTable
    #tab_group_class = admin_tabs.PCIeSwitchTabs
    template_name = 'admin/drawers/index.html'
    page_title = _("Drawers")
    ...
    def get_data(self):
        ...
        try:
            ...
            drawers_list = api.rsa.drawers_list(request, pod_id)
        except:
...
class DetailView(tabs.TabView):
    tab_group_class = admin_tabs.DrawerDetailTabs
    template_name = 'admin/drawers/_detail.html'
    #redirect_url = 'horizon:project:instances:index'
    page_title = _("Drawer Details: {{ drawer.name }}")
    ...
    @memoized.memoized_method
    def get_data(self):
        drawer_id = self.kwargs['drawer_id']
        drawer = {}
        try:
            try:
                if 'SELECTED_POD_MANAGER_ID' not in self.request.session:
                    ...
                    pod_id = self.request.session['SELECTED_POD_MANAGER_ID']
                drawers_list = api.rsa.drawers_list(self.request, pod_id)
                blades = api.rsa.blades_list(self.request, pod_id)
                drawer_id_list  = [ d['id'] for d in drawers_list ]
                blades_id_list  = [ b['id'] for b in blades ]
            except:
                exceptions.handle(self.request,
                                _('Unable to retrieve drawer list.'))
            result = api.rsa.get_drawer_by_id(self.request, pod_id , drawer_id)
            drawer = result['data']
            if eval(drawer['location']) == {}:
                drawer['location'] = ''
                drawer['rack'] = ''
            else:
                if 'Rack' in eval(drawer['location']):
                    drawer['rack'] = eval(drawer['location'])['Rack']
                if 'ULocation' in eval(drawer['location']):
                    drawer['location'] = eval(drawer['location'])['ULocation']
            if 'Health' in eval(drawer['status']):
                drawer['status'] = eval(drawer['status'])['Health']
        except:
            import traceback
            traceback.print_exc()
            if int(drawer_id) == 1:
    ...
    def get_tabs(self, request, *args, **kwargs):
        drawer = self.get_data()
        return self.tab_group_class(request, drawer=drawer, **kwargs)
