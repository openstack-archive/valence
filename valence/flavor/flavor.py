...
def get_available_criteria():
    ...
    for pfile in pluginfiles:
        module = import_module("valence.flavor.plugins." + pfile)
        myclass = getattr(module, pfile + 'Generator')
        inst = myclass([])
        resp.append({'name': pfile, 'description': inst.description()})
...
def create_flavors(data):
    ...
    for criteria_name in criteria.split(","):
        if criteria_name:
            ...
            module = __import__("valence.flavor.plugins." + criteria_name, fromlist=["*"])
