...
class ExpEtherManager(object):
    ...
    def create_compose_request(self, name, description, requirements):
        ...
        if 'pci_device' in requirements:
            if 'type' in requirements['pci_device']:
                pci_device['type'] = requirements['pci_device']['type']
        request['PCIDevice'] = pci_device
        return request
    ...
    def compose_node(self, request_body):
        ...
        Sample Request: {'name': 'node1',
                         'properties': {
                             'pci_device': {'type': ['NIC', 'SSD']}
                         }}
        ...
        if request_body.get('PCIDevice'):
            req_pci_type = request_body['PCIDevice']['type']
    ...
    def manage_node(self, node_index):
        """To manage existing nodes via valence

        It retrieve eesv info required to manage node
    ...
    def get_status(self):
        ...
        """
        exc_message = "unable to reach podmanager at url: {} with error: {}"
        try:
        ...
        # check for wrong ip and offline status
            ...
            raise exception.AuthorizationFailure(
                detail=exc_message.format(self.url, e.detail))
        except exception.ExpEtherException as e:
            raise exception.ExpEtherException(
                code=e.code, detail=exc_message.format(self.url, e.detail),
                status=e.status)
    ...
    def _get_device_by_mac_id(mac_id):
        """Retrieve device from DB based on mac address

        :param mac_id: mac address to search device
        :return: device in dict
        """
    ...
    def _get_device_by_device_id(device_id):
        """Retrieve device from DB based on device id

        :param device_id: mac address to search device
        :return: device in dict
        """
        dev_list = db_api.Connection.list_devices()
        device = [dev for dev in dev_list
